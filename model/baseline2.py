...
## This baseline breaks down parsers. Instead of averaging the score of all parsers, we consider indivitual parser and image pair,
## (parser, image)

class baseline_model2():
    def __init__(self, batch_size, feature_dim, embed_dim, n_lstm_steps, word_num, phrs_size_all, phrs_size_stepByCls):
        ...
        self.Wxt = self.init_weight([self.feats_dim, self.lstm_hidden_dim * 2],
                                            name='Wxt')
    ...
    def get_loss(self, pred_score, groundtruth):
        cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(pred_score, groundtruth, name=None)
    ...
    def model(self):
        ...
        labels = tf.placeholder("float32", [self.batch_size, self.phrase_size])
        ...
        return image_feat, phrase_feat, labels,  score_mat
    ...
    def trainer(self):
        ...
        acc = loss # dummy
        #max_prob_words = tf.argmax(avg_loss, 1)
